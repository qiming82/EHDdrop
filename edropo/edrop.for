...
c      dimension sumxx(neq,neq), sumxr(neq,neq), sumrx(neq,neq)
	...
	eps = 0.0000000001D0
      ...
      open (15,file='fm.dat')
      open (31,file='def.dat')
      open (32,file='xyvalss.dat')
      open (33,file='fcurv.dat')
...
c	   call regridfadp(znew,rnew,z,r,s,N,time,dt)
c           call regridfadp2(znew,rnew,z,r,s,N,vnmax,dt)
...
c	       write(7,*) "velocity too small"
c             write(7,*) 'vmax_min=', vnmax
c	       exit
c	    end if
          if(dabs(vnmax).lt.0.0004d0)then
	   RL=z(N+1)
	   B =r(N/2+1)
	   D=(RL-B)/(RL+B)
             if(eb.gt.0.36d0)then
		  write(7,*) 'eb=', eb   
		 write(31,*) eb, D
	        do j=1,N+1
	          write(32,*) z(j), r(j)
