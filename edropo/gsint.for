...
c	write (*,*) 'r1=', r1
	...
	do j=1,8
     ...
     +		     ,E21(j),E22(j),iaxis)
c          call sgf_ax_g(iopt,zg(j),rg(j),-z0,r0,G11(j),G12(j)
c     +	         ,G21(j),G22(j),iaxis)
      end do
c -------
...
c	     xx_tmp2 = xx_tmp2 - a*G11(j)*qxg(j)*w1(j)
	     
	     yy_tmp1 = yy_tmp1 + a*E22(j)*qyg(j)*w1(j)
c	     yy_tmp2 = yy_tmp2 + a*G22(j)*qyg(j)*w1(j)
...
c        if (z0.lt.eps) then
c	     y11 = 0.0D0
c	     y22 = 2.0D0*(yy_tmp1+2.0D0*(yy2+yy3))
c	   else
