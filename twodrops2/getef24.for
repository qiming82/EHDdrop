...
c----
	 beta1 = 1.0-RR1
	 beta2 = RR1/RR2*(1.0-RR2)
	 betah1 = 1.0-RR2
	 betah2 = RR2/RR1*(1.0-RR1)
	 gma1 = -beta1
	 gma2 = -beta2
	 gmah1 = -betah1
	 gmah2 = -betah2
c----
c	 H11 = H11*2.0d0*(1.0-RR)/(1.0+RR)
c	 H12 = H12*2.0d0*(1.0-RR)/(1.0+RR)
c	 H21 = H21*2.0d0*(1.0-RR)/(1.0+RR)
c	 H22 = H22*2.0d0*(1.0-RR)/(1.0+RR)

	 H11 = H11*2.0d0/(1.0+RR1)*beta1
	 H12 = H12*2.0d0/(1.0+RR1)*beta2
	 H21 = H21*2.0d0/(1.0+RR2)*betah1
	 H22 = H22*2.0d0/(1.0+RR2)*betah2
...
c--
	do j=1,N+1
	  tmp1=0.0d0
          tmp2=0.0d0
	  do k=1,N+1
c	    tmp1=tmp1+F11(j,k)*anz1(k)+F12(j,k)*anz2(k) 
c	    tmp2=tmp2+F21(j,k)*anz1(k)+F22(j,k)*anz2(k) 
	   tmp1=tmp1+F11(j,k)*anz1(k)*gma1+F12(j,k)*anz2(k)*gma2 
	   tmp2=tmp2+F21(j,k)*anz1(k)*gmah1+F22(j,k)*anz2(k)*gmah2
	  enddo
c         RHS(j) = -2.0*(RR-1.0d0)/(RR+1.0d0)*tmp1
c         RHS(j+N+1) = -2.0*(RR-1.0d0)/(RR+1.0d0)*tmp2
         RHS(j) = -2.0/(RR1+1.0d0)*tmp1
         RHS(j+N+1) = -2.0/(RR2+1.0d0)*tmp2
	enddo
...
c--- UP TO HERE      
	do j=1,N+1
         RHS(j) = phih1(j)/(RR1-1.0d0)  ! RR ne 1
         RHS(j+N+1) = phih2(j)/(RR2-1.0d0)  ! RR ne 1
	end do
