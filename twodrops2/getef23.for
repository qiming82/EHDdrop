...
c--
c	 H11 = H11*2.0d0*(1.0-RR1)/(1.0+RR1)
c	 H12 = H12*2.0d0*(1.0-RR)/(1.0+RR)
c	 H21 = H21*2.0d0*(1.0-RR)/(1.0+RR)
c	 H22 = H22*2.0d0*(1.0-RR)/(1.0+RR)
...
c--
	...
	enddo
       do i=1,N+1
         do k=1,N+1
           Am(i,k+nn2)= -(RR1-1.0)*F11(i,k)
           Am(i,k+nn3)= -(RR2-1.0)*F12(i,k)
           Am(i+nn1,k+nn2)= -(RR1-1.0)*F21(i,k)
           Am(i+nn1,k+nn3)= -(RR2-1.0)*F22(i,k)
           Am(i+nn2,k) = 2.0*coef1*H11(i,k)
           Am(i+nn2,k+nn1) = 2.0*coef1*H12(i,k)
c           Am(i+nn2,k+nn3) = -2.0*(RR2-RR1)/(1.+RR1)*F12(i,k)
           Am(i+nn3,k) = 2.*coef2*H21(i,k)
           Am(i+nn3,k+nn1) = 2.*coef2*H22(i,k) 
c           Am(i+nn3,k+nn2) = -2.0*(RR1-RR2)/(1.+RR2)*F21(i,k)
         enddo
       enddo

	 if(abs(RR2-RR1).lt.0.00001)then
	    do i=1,N+1
	      do k=1,N+1
	     Am(i+nn2,k+nn3) = -2.0*(RR2-RR1)/(1.+RR1)*F12(i,k)
             Am(i+nn3,k+nn2) = -2.0*(RR1-RR2)/(1.+RR2)*F21(i,k)
	      enddo
	    enddo
	 else
	    do i=1,N+1
...
c--
	...
	do i=1,N+1
	    Am(i,i) = 1.0
	    Am(i+nn1,i+nn1) = 1.0
	    Am(i+nn2,i)=1.0d0 + Am(i+nn2,i)
	    Am(i+nn3,i+nn1)=1.0d0 + Am(i+nn3,i+nn1)
	enddo

c--- linear sys unknowns: phih1, phih2, phi3n1, phi3n2
c--  for RHS
	...
	do j=1,N+1
	 ...
	 RHS(j)=0.0
	 RHS(j+nn1)=0.0
         RHS(nn2+j) = -2.0*(RR1-1.0d0)/(RR1+1.0d0)*tmp1
         RHS(nn2+j+N+1) = -2.0*(RR2-1.0d0)/(RR2+1.0d0)*tmp2
	enddo

         call dgesv(4*N+4, 1, Am, neq, ipiv, RHS, neq, info)
        phih1(1:N+1)=RHS(1:N+1)   ! surface potential phih = phis + z
        phih2(1:N+1)=RHS(N+2:2*N+2)
        dpdn1(1:N+1)=RHS(nn2+1:nn2+N+1)   !dphidn_3(1)
        dpdn2(1:N+1)=RHS(nn3+1:4*N+4)   !dphidn_3(2)
...
c	do j=1,N+1
c         RHS(j) = phih1(j)/(RR-1.0d0)  ! RR ne 1
c         RHS(j+N+1) = phih2(j)/(RR-1.0d0)  ! RR ne 1
c	end do

c         call dgesv(2*N+2, 1, Am, neq, ipiv, RHS, neq, info)
c      dpdn1(1:N+1)=RHS(1:N+1)   !dphidn_2
c      dpdn2(1:N+1)=RHS(N+1+1:2*N+2)   !dphidn_2
c--
      Return 
      END
c-----------------------------------------------------------------------------
