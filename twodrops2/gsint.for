	...
	dimension x1(10),x2(10),w1(10),w2(10)
      dimension rg(10),rs(10),zg(10),pf1(10),pf2(10)
	dimension sg(10),xh2(10),rh2(10),akg(10),anyg(10),anxg(10)
      dimension ss(10),qxg(10),qyg(10),zs(10),aks(10)
	dimension qxs(10),qys(10),pe1(10),pe2(10)
	dimension E11(10), E12(10), E21(10), E22(10)
c      dimension G11(10), G12(10), G21(10), G22(10)
	...
	do j=1,8
	   ...
	   call sgfax (rg(j),zg(j),r0,z0,
     +	   E11(j),E12(j),E21(j),E22(j))
      end do
c -------
	  ...
	  do j=1,8
           call G_singlr (rg(j),zg(j),r0,z0,sg(j),s0,
     +		 E11(j),E22(j))
        end do	   
...
c  singular part
     ...
     +              (r02-rs(j)**2)**2)/rh2(j)*Pee2)*qys(j)*w2(j)
        ...
        yy3 = yy3*(s2-s1)*2.0D0
	     ...
	     y11 = xx_tmp1 + 2.0D0*(xx2 + xx3)
	     y22 = yy_tmp1 + 2.0D0*(yy2 + yy3)
...
c  singular part
	...
	sum3 = 0.0D0
      ...
      do j=1,8
	  sum2 = sum2 + a*E12(j)*qyg(j)*w1(j)
	  sum3 = sum3 + a*E21(j)*qxg(j)*w1(j)
      end do
...
c---------------------------------------------------
      subroutine G_singlr(Y,X,Y0,X0,s,s0,SXX,SYY)
...
c	end if
	...
	Rd = dsqrt(xh2+(Y+Y0)**2)
	SXX = 4.0D0*Y/Rd*(F+xh2/rh2*E)
	if (Y0.lt.eps) then
      	SYY = 0.0 D0
      else
	    SYY = 2.0D0/(Y0*Rd)*((Y02+Y2+2.0D0*xh2)*F - (2.0D0*xh2**2
     +	     + 3.0D0*xh2*(Y2+Y02)+(Y02-Y2)**2)/rh2*E) 
      end if
	...
	END
c---------------------------------------------------------
      Subroutine sgfax (r,z,r0,z0,Gxx,Gxy,Gyx,Gyy)
