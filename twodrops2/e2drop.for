...
c        qr2(j)= (z2(j)+(a1/a2)**2*Boi*f2(j))*anr2(j)
        ...
        do j=1,N
         ...
         tmp3 = (tmp1+tmp2*uz1(j))*r1(j)
          tmp1 = un1(j+1)*z1(j+1)
        tmp2 = an11(j+1)*z1(j+1)+an12(j+1)*r1(j+1)
         tmp4 = (tmp1+tmp2*uz1(j+1))*r1(j+1)
         umove1=umove1+htmp*(tmp3+tmp4)
       end do
       umove1=umove1*pi/cvol1
         umove2=0.0D0
        do j=1,N
          htmp=z2(j+1)-z2(j)
          tmp1 = un2(j)*z2(j)
          tmp2 = an21(j)*z2(j)+an22(j)*r2(j)
         tmp3 = (tmp1+tmp2*uz2(j))*r2(j)
          tmp1 = un2(j+1)*z2(j+1)
          tmp2 = an21(j+1)*z2(j+1)+an22(j+1)*r2(j+1)
        tmp4 = (tmp1+tmp2*uz2(j+1))*r2(j+1)
         umove2=umove2+htmp*(tmp3+tmp4)
       end do
       umove2=umove2*pi/cvol2

c       write (*,*) time, umove1, umove2
c---- try surface area average of tangential velocity
         utmove1=0.0D0
        do j=1,N
          htmp=z1(j+1)-z1(j)
          tmp1 = ut1(j)*r1(j)
          tmp2 = ut1(j+1)*r1(j+1)
          utmove1=utmove1+htmp*(tmp1+tmp2)/2.0
        enddo
	utmove1 = utmove1/2.0/a1**2
        ...
        do j=1,N
          ...
          tmp1 = ut2(j)*r2(j)
          tmp2 = ut2(j+1)*r2(j+1)
          utmove2=utmove2+htmp*(tmp1+tmp2)/2.0
        enddo
	utmove2 = utmove2/2.0/a2**2
...
c	  u2dep= (-2.)*coeffth/dh**4
	 ...
	 dh2 = dh/2.0
	 call zabarankU(a1,a2,dh2,RR1,RR2,eb,ram1,ram2,vtheo)
c	 write(*,*) a1,a2,dh2,RR1,RR2,eb,ram1,ram2,vtheo
c	 write(*,*) vtheo
c	 exit
...
c              write (43,*) time, utmove1, u1theory
              write (43,*) time, umove1, vtheo
              write (44,*) time, utmove2, u2theory
              write (32,*) time, da
